...
namespace Mt.GraphQL.Api.Test
{
    public class FilterDeserializerTests
    {
        ...
        [TestCase("startsWith(Description,'Ent''ity')", "x => x.Description.StartsWith(\"Ent'ity\")")]
        ...
        [TestCase(
            ...
            "x => (((x.Id != 3) AndAlso x.Description.Contains(\"A\")) OrElse x.Description.EndsWith(\"B\"))")]
        [TestCase(
        ...
        [TestCase("Id in (1", "QueryException", "Expected closing parenthesis at position 8")]
        [TestCase("Id in 1)", "QueryException", "Could not parse filter from position 7")]
